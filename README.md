# 🏥 Disease Outbreak Prediction - MedPredict
![Home Page](images/home.jpg)

## 📌 Overview
The **Disease Outbreak Prediction** project is used to predict **heart/diabetes disease** risk based on patient data. It uses **machine learning models** to analyze health indicators and provide risk assessments. The application helps users monitor their heart health and take preventive measures early.

## 📋 Features
- 🏠 **Home Page:** A user-friendly interface for easy navigation.
- 🔍 **Heart Disease Prediction:** Predicts heart disease risk using patient data.
- 🔬 **Diabetes Disease Prediction:** Predicts Diabetes disease risk using patient data.
- 📊 **Machine Learning Model:** Uses AI to analyze key health indicators.
- 🏥 **Health Insights:** Provides recommendations based on predictions.
![Heart Disease Prediction Model](images/heart.jpg)
![Diabetes Disease Prediction Model](images/diabetes.jpg)


## 🏥 How It Works
1. **User inputs health metrics** (Age, Blood Pressure, Cholesterol, etc.)
2. **ML Model processes the input** and predicts heart/diabetes disease risk.
3. **Results are displayed with insights and recommendations.**

## 📸 Screenshots
- **Home Page:** `images/home.jpg`
- **Prediction Page:** `images/heart.jpg`
- **About Page:** `images/about.jpg`
![About](images/about.jpg)

## 🤝 Contributing
Feel free to **fork** the repository and submit **pull requests**. Contributions are welcome!

## 📄 License
This project is **MIT Licensed**. See `LICENSE` for details.

## 📩 Contact
For any queries, reach out to: https://satish-tec.github.io/
